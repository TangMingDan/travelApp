...
public class CameraHanlder implements View.OnClickListener {
    ...
    private final PermissionCheckrDelegate DELEGATE;

    public CameraHanlder(PermissionCheckrDelegate delegate) {
        this.DIALOG = new AlertDialog.Builder(delegate.getContext()).create();
        this.DELEGATE = delegate;
    }

    final void showCameraDialog(){
        DIALOG.show();
        final Window window = DIALOG.getWindow();
        Log.d("123456","显示dialog");
        if (window != null) {
    ...
    private String getPhotoName(){
        return MyFileUtils.getFileNameByTime("IMG","jpg");
    }

    private void takePhoto(){
        final String currentPhotoName = getPhotoName();
        final Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);
        final File tempFile = new File(MyFileUtils.CAMERA_PHOTO_DIR,currentPhotoName);
        ...
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.N){
            ...
            //需要将Uri路径转化为实际路径
            final File realFile = FileUtils.getFileByPath(MyFileUtils.getRealFilePath(DELEGATE.getContext(),uri));
